...
export default function DaveClientPage() {
  ...
  const totalMonthlySubscriptions = subscriptions?.filter((s: any) => s.active)
    .reduce((sum: number, sub: any) => sum + Number(sub.monthly_price), 0) || 0

  const todaySpend = summary?.today?.totalUsd || 0
  const weekSpend = summary?.week?.totalUsd || 0
  const monthSpend = summary?.month?.totalUsd || 0

  // Calculate combined totals (AI + subscriptions)
  const monthlyBudgetTotal = monthSpend + totalMonthlySubscriptions
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="bg-card/50 border-border">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Tier 3: ${((summary?.week?.byTier?.tier3 || 0) / 100).toFixed(2)}
              </p>
            </CardContent>
          </Card>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground mt-1">
                          ${(alert.spentCents / 100).toFixed(2)} / ${(alert.limitCents / 100).toFixed(2)}
              ...
              {summary?.today?.byAgent?.map((agent: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-background/50 border border-border">
                  ...
                  <div className="text-right">
                    <div className="font-mono text-foreground">${agent.costUsd.toFixed(2)}</div>
                  </div>
                </div>
              ))}
          ...
          <Card className="bg-card/50 border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-400">
                ${((summary?.month?.byTier?.tier1 || 0) / 100).toFixed(2)}
              </div>
          ...
          <Card className="bg-card/50 border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-400">
                ${((summary?.month?.byTier?.tier2 || 0) / 100).toFixed(2)}
              </div>
          ...
          <Card className="bg-card/50 border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-400">
                ${((summary?.month?.byTier?.tier3 || 0) / 100).toFixed(2)}
              </div>
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono text-foreground">${monthSpend.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Subscriptions (Monthly)</span>
                <span className="font-mono text-foreground">${totalMonthlySubscriptions.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">xAI Balance Remaining</span>
                <span className="font-mono text-green-400">${xaiBalance?.balance_usd?.toFixed(2) || '0.00'}</span>
              </div>
              <div className="border-t pt-2 mt-2">
                <div className="flex justify-between font-semibold">
                  <span>Total Monthly Spend</span>
                  <span className="font-mono text-foreground text-lg">${monthlyBudgetTotal.toFixed(2)}</span>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent>
            ...
            <div className="mt-4 pt-4 border-t">
              <div className="flex justify-between font-semibold">
                ...
                <span className="font-mono">${totalMonthlySubscriptions.toFixed(2)}</span>
              </div>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          ...
                          <div className="mt-1">
                            Spent: ${(alert.spentCents / 100).toFixed(2)} / ${(alert.limitCents / 100).toFixed(2)}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
