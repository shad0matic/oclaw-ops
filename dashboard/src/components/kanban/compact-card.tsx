...
export function CompactTaskCard({ task, projects, onClick }: TaskCardProps) {
  ...
  const getCommentStatus = (task: QueueTask): CommentStatus | null => {
      ...
      // Chat was acked with thumbs up - hide icon entirely
      // (trigger clears chat_acked_at on new comments, so icon reappears then)
      ...
      if (lastCommenterIsBoss) {
        ...
        if (lastComment.read_at) {
          return 'read';    // Dim blue - agent saw but didn't reply
        } else {
